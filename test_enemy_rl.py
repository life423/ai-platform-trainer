...
def test_environment_creation():
    ...
    try:
        from ai_platform_trainer.ai.training.enemy_rl_environment import (
            EnemyRLConfig,
            EnemyRLEnvironment,
        )
...
def test_enemy_entity():
    ...
    try:
        ...
        for step in range(5):
            ...
            print(
                f"   Step {step+1}: Position ({pos['x']:.1f}, {pos['y']:.1f}), Energy: {stats['energy']:.1f}"
            )
...
def test_reward_function():
    ...
    try:
        ...
        scenarios = [
            {
                "name": "Safe Movement",
                "action": [0.1, 0.1],
                "expected": "Positive survival reward",
            },
            {
            ...
            },
            {
                "name": "No Movement",
                "action": [0.0, 0.0],
                "expected": "Conservative reward",
            },
            {
                "name": "Evasive Movement",
                "action": [0.5, -0.5],
                "expected": "Mixed reward",
            },
        ]
        ...
        for scenario in scenarios:
            ...
            print(
                f"     Info: Survival time {info['survival_time']}, Energy {info['enemy_energy']:.1f}"
            )
...
def demonstrate_rl_vs_scripted():
    ...
    try:
        ...
        for enemy_type, enemy in [("RL", rl_enemy), ("Scripted", scripted_enemy)]:
            ...
            print(
                f"   {enemy_type} Enemy: Moved ({movement['x']:.2f}, {movement['y']:.2f})"
            )
...
def main():
    ...
    if passed == total:
        ...
        print("\\n🚀 Next Steps:")
        print(
            "   1. Train full enemy RL model: python -m ai_platform_trainer.ai.training.enemy_rl_environment"
        )
