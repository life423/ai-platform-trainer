    ...
    try:
        ...
        for step in range(5):
            ...
            try:
                ...
                print(
                    f"✅ Step completed - Reward: {reward:.3f}, Terminated: {terminated}"
                )
                print(f"   Active missiles: {len(env.missiles)}")
                print(
                    f"   Enemy position: ({env.enemy_pos['x']:.1f}, {env.enemy_pos['y']:.1f})"
                )

                # Manual missile inspection
                for i, missile in enumerate(env.missiles):
                    print(
                        f"   Missile {i}: pos=({missile.pos['x']:.1f}, {missile.pos['y']:.1f}), vx={missile.vx:.1f}, vy={missile.vy:.1f}"
                    )
                ...
                for i, missile in enumerate(env.missiles):
                    ...
                    print(
                        f"   Missile {i} dir: {[attr for attr in dir(missile) if not attr.startswith('_')]}"
                    )
                break
...
def main():
    ...
    tests = [test_missile_creation, test_missile_collision]
