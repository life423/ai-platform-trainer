...
class MissileRLEnvironment(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.missile = Missile(
            missile_x, missile_y, speed=8.0, vx=initial_vx, vy=initial_vy
        )
        ...
        self.steps = 0
        self.max_steps = (
            600  # 10 seconds at 60 FPS - more time to learn complex maneuvers
        )
    ...
    def _calculate_reward(self) -> float:
        ...
        if (
            self.missile.pos["x"] < 0
            or self.missile.pos["x"] > self.screen_width
            or self.missile.pos["y"] < 0
            or self.missile.pos["y"] > self.screen_height
        ):
    ...
    def _is_terminated(self) -> bool:
        ...
        if (
            self.missile.pos["x"] < -50
            or self.missile.pos["x"] > self.screen_width + 50
            or self.missile.pos["y"] < -50
            or self.missile.pos["y"] > self.screen_height + 50
        ):
            return True
...
class MissileRLTrainer:
    ...
    def test_model(self, model_path: str, num_episodes: int = 10):
        ...
        logging.info(
            f"Test Results: Avg Reward = {avg_reward:.2f}, Hit Rate = {hit_rate:.2%}"
        )
...
def main():
    ...
    if not STABLE_BASELINES_AVAILABLE:
        print(
            "stable_baselines3 is not available. Please install it to run RL training."
        )
        return
