...
class MissileSACEnvironment(gym.Env):
    ...
    def step(self, action):
        ...
        turn_rate = np.clip(
            action[0] * self.max_turn_rate, -self.max_turn_rate, self.max_turn_rate
        )
    ...
    def _calculate_sac_reward(self, action: float) -> float:
        ...
        if (
            np.linalg.norm(intercept_vector) > 0
            and np.linalg.norm(missile_velocity) > 0
        ):
...
class MissileSACTrainer:
    ...
    def train(
                ...
                if len(self.locals.get("infos", [])) > 0:
                    ...
                    if "episode" in info:
                        ...
                        if len(self.episode_rewards) >= 10:
                            ...
                            logging.info(
                                f"Recent 10 episodes: Avg reward = {recent_reward:.2f}, Avg length = {recent_length:.1f}"
                            )
        ...
        # Train the model
        logging.info(
            f"Starting missile SAC training for {total_timesteps} timesteps..."
        )
        logging.info(
            "SAC advantages: Off-policy learning, sample efficiency, continuous control"
        )
...
def main():
    ...
    if not STABLE_BASELINES_AVAILABLE:
        print(
            "stable_baselines3 is not available. Please install it to run SAC training."
        )
        return
    ...
    logging.info(
        "Training with SAC (Soft Actor-Critic) - optimized for continuous control"
    )
    model = trainer.train(
        total_timesteps=30000
    )  # Less than PPO due to sample efficiency
