...
class EnhancedSACTrainer:
    ...
    def __init__(
        self,
        save_path: str = "models/enhanced_missile_sac",
    ...
    def train(
        ...
        if self.use_curriculum:
            curriculum_callback = CurriculumProgressCallback(
                curriculum_manager=self.curriculum_manager, env=env, eval_env=eval_env
            )
        ...
        logging.info(
            f"🚀 Starting enhanced SAC training for {total_timesteps} timesteps"
        )
        if self.use_curriculum:
            initial_stage = self.curriculum_manager.get_current_stage()
    ...
    def _generate_training_report(self):
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                f"- **Improvement**: {summary.get('improvement_trend', 0):.2%}\\n\\n"
            )
            ...
            if self.curriculum_manager:
                ...
                f.write(
                    f"- **Current Stage**: {progress['current_stage']} ({progress['stage_index']}/{progress['total_stages']})\\n"
                )
                f.write(f"- **Stage Success Rate**: {progress['success_rate']:.2%}\\n")
                f.write(
                    f"- **Target Success Rate**: {progress['target_success_rate']:.2%}\\n"
                )
            ...
            if summary.get("best_hit_rate", 0) > 0.8:
                ...
                f.write(
                    "✅ **Good Performance**: Solid hit rate with room for improvement\\n"
                )
            else:
                f.write(
                    "⚠️ **Needs Improvement**: Consider adjusting reward function\\n"
                )
    ...
    def test_model(
        self, model_path: str, num_episodes: int = 20, test_all_stages: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        if test_all_stages and self.curriculum_manager:
            ...
            for stage in self.curriculum_manager.stages:
                ...
                stage_results = self._test_on_stage(
                    model, test_env, num_episodes, stage.name
                )
                results[stage.name] = stage_results
        else:
            # Test on current/default stage
            test_env = EnhancedMissileEnvironment()
            results["standard"] = self._test_on_stage(
                model, test_env, num_episodes, "standard"
            )
    ...
    def _test_on_stage(
        self, model, env, num_episodes: int, stage_name: str
    ) -> Dict[str, Any]:
    ...
    def _print_test_summary(self, results: Dict[str, Any]):
        ...
        for stage_name, metrics in results.items():
            ...
            logging.info(
                f"   Trajectory Efficiency: {metrics['avg_trajectory_efficiency']:.2f}"
            )
            logging.info(
                f"   Oscillation Count: {metrics['avg_oscillation_count']:.1f}"
            )
...
def run_enhanced_training():
    ...
    if not STABLE_BASELINES_AVAILABLE:
        print(
            "stable_baselines3 is not available. Please install it for enhanced training."
        )
        return
    ...
    results = trainer.test_model(
        "models/enhanced_missile_sac_final.zip", num_episodes=15
    )

    print("\\n✅ Enhanced training completed!")
    print(
        "Check 'enhanced_missile_analysis/' for detailed reports and trajectory visualizations"
    )
