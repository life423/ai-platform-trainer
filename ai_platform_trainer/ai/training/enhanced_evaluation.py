...
class BehaviorAnalysisCallback(BaseCallback):
    ...
    def _run_comprehensive_evaluation(self):
        ...
        logging.info(
            f"Running comprehensive evaluation at timestep {self.num_timesteps}"
        )
        ...
        current_stage = (
            self.curriculum_manager.get_current_stage().name
            ...
            if done:
                ...
                if not info["success"]:
                    if step_count >= 1000:
                        ...
                        self.eval_env.missile.pos["x"] < 0
                        or self.eval_env.missile.pos["x"] > self.eval_env.screen_width
                    ):
    ...
    def _analyze_behavior_patterns(
        ...
        near_misses = [
            ep["final_distance"] < 30 and not ep["success"] for ep in episode_data
        ]
        ...
        avg_time_to_hit = (
            np.mean([ep["episode_length"] for ep in successful_episodes])
        ...
        common_failure_modes = sorted(
            failure_mode_counts.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def _generate_recommendations(self, analysis: BehaviorAnalysis) -> List[str]:
        ...
        if analysis.hit_rate < 0.5:
            recommendations.append(
                "🎯 Low hit rate - consider increasing hit reward or decreasing success threshold"
            )

        # Near miss analysis
        if analysis.near_miss_rate > 0.3:
            recommendations.append(
                "🎯 High near-miss rate - missile gets close but doesn't hit. Consider tighter success threshold or stronger final approach reward"
            )

        # Timeout analysis
        if analysis.timeout_rate > 0.4:
            recommendations.append(
                "⏱️ High timeout rate - increase time penalty or reduce episode length"
            )

        # Oscillation analysis
        if analysis.avg_oscillation_count > 8:
            recommendations.append(
                "🌊 High oscillation - increase oscillation penalty or smooth progress rewards more"
            )

        # Trajectory efficiency analysis
        if analysis.avg_trajectory_efficiency < 0.6:
            recommendations.append(
                "📈 Low trajectory efficiency - increase efficiency bonus or add more directional guidance"
            )

        # Episode length analysis
        if analysis.avg_time_to_hit > 400:
            recommendations.append(
                "🏃 Slow interception - increase time penalty or add speed bonus"
            )

        # Failure mode analysis
        if "near_miss" in analysis.common_failure_modes:
            recommendations.append(
                "🎯 Common near misses - fine-tune final approach rewards"
            )

        if "oscillation" in analysis.common_failure_modes:
            recommendations.append(
                "🌊 Oscillation failures - reduce progress reward weight or increase smoothing"
            )

        if "timeout" in analysis.common_failure_modes:
            recommendations.append(
                "⏱️ Timeout failures - increase urgency signals or time penalties"
            )

        if not recommendations:
            recommendations.append(
                "✅ Behavior looks good! Consider advancing difficulty or fine-tuning hyperparameters"
            )
    ...
    def _save_trajectory_plots(self, report: EvaluationReport):
        ...
        fig.suptitle(
            f"Missile Behavior Analysis - Timestep {report.timestep}", fontsize=14
        )
        ...
        if self.curriculum_manager:
            ...
            for _ in range(self.n_eval_episodes):
                ...
                if advanced:
                    ...
                    logging.info(
                        f"📚 Advanced to new curriculum stage: {new_stage.name}"
                    )
                    break
    ...
    def _log_evaluation_summary(self, report: EvaluationReport):
        ...
        logging.info(
            f"   Trajectory Efficiency: {analysis.avg_trajectory_efficiency:.2f}"
        )
        ...
        if analysis.common_failure_modes:
            logging.info(
                f"   Common Failures: {', '.join(analysis.common_failure_modes)}"
            )
    ...
    def get_training_summary(self) -> Dict[str, Any]:
        ...
        hit_rates = [
            report.behavior_analysis.hit_rate for report in self.evaluation_reports
        ]
        ...
        return {
            ...
            "improvement_trend": hit_rates[-1] - hit_rates[0]
            if len(hit_rates) > 1
            else 0,
            "timesteps": timesteps,
            "hit_rates": hit_rates,
            "curriculum_completed": (
                self.curriculum_manager.current_stage_idx
                == len(self.curriculum_manager.stages) - 1
            )
            if self.curriculum_manager
            else False,
        }
