    ...
    def _on_step(self) -> bool:
        ...
        if len(self.locals.get("infos", [])) > 0:
            ...
            if "episode" in info:
                ...
                if (
                    len(self.episode_successes) >= 100
                    and self.convergence_timestep is None
                ):
...
class MissileRLBenchmark:
    ...
    def benchmark_single_config(
        self, config: AlgorithmConfig, test_episodes: int = 50
    ) -> BenchmarkResult:
        ...
        try:
            ...
            timesteps_to_convergence = (
                tracker.convergence_timestep or config.expected_timesteps
            )
            ...
            result = BenchmarkResult(
                ...
                final_episode_reward=np.mean(tracker.episode_rewards[-10:])
                if tracker.episode_rewards
                else 0.0,
                training_success_rate=np.mean(tracker.episode_successes[-100:])
                if len(tracker.episode_successes) >= 100
                else 0.0,
            ...
            self.logger.info(
                f"Completed benchmark for {config.name} in {time.time() - start_time:.1f}s"
            )
            return result
    ...
    def run_comparison(
        self, configs: List[AlgorithmConfig], test_episodes: int = 50
    ...
    def _generate_report(self, results: List[BenchmarkResult]):
        ...
        results_sorted = sorted(
            results, key=lambda x: x.sample_efficiency_score, reverse=True
        )
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                "| Algorithm | Hit Rate | Avg Reward | Sample Efficiency | Training Time | Inference Time |\\n"
            )
            f.write(
                "|-----------|----------|------------|-------------------|---------------|----------------|\\n"
            )
            ...
            for result in results_sorted:
                f.write(
                    f"| {result.config_name} | {result.eval_hit_rate:.2%} | "
            ...
            f.write(
                f"- **Convergence**: {best.timesteps_to_convergence:,} timesteps\\n\\n"
            )
            ...
            if sac_results:
                ...
                f.write(
                    f"**SAC Performance**: Best hit rate {best_sac.eval_hit_rate:.2%} "
            ...
            if ppo_results:
                ...
                f.write(
                    f"**PPO Performance**: Best hit rate {best_ppo.eval_hit_rate:.2%} "
            ...
            if sac_results and ppo_results:
                avg_sac_efficiency = np.mean(
                    [r.sample_efficiency_score for r in sac_results]
                )
                avg_ppo_efficiency = np.mean(
                    [r.sample_efficiency_score for r in ppo_results]
                )

                if avg_sac_efficiency > avg_ppo_efficiency:
                    f.write(
                        "- **SAC is more sample efficient** for missile guidance tasks\\n"
                    )
                    f.write(
                        "- Consider using SAC for faster training and better continuous control\\n"
                    )
                else:
                    f.write("- **PPO remains competitive** despite being on-policy\\n")
                    f.write(
                        "- PPO may be more stable for some hyperparameter configurations\\n"
                    )
    ...
    def plot_training_curves(self, results: List[BenchmarkResult]):
        ...
        colors = [
            "blue" if "SAC" in alg else "orange" if "PPO" in alg else "green"
        ...
        scatter = ax4.scatter(
            ...
            c=[
                "blue"
                if "SAC" in r.config_name
                else "orange"
                if "PPO" in r.config_name
                else "green"
                for r in results
            ],
            s=100,
...
def run_comprehensive_benchmark():
    ...
    if not STABLE_BASELINES_AVAILABLE:
        print(
            "stable_baselines3 is not available. Please install it to run benchmarks."
        )
        return
