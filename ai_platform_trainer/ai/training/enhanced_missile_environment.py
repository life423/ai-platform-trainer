...
class EnhancedMissileEnvironment(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.missile = Missile(
            missile_x, missile_y, speed=initial_speed, vx=initial_vx, vy=initial_vy
        )
    ...
    def _create_curriculum_target(self):
        ...
        while (
            np.sqrt(
                (target_x - self.missile.pos["x"]) ** 2
                + (target_y - self.missile.pos["y"]) ** 2
            )
            < 200
        ):
            target_x = np.random.uniform(100, self.screen_width - 100)
    ...
    def _calculate_enhanced_reward(self, action_value: float) -> float:
        ...
        if len(self.distance_history) >= self.progress_smoothing_window:
            ...
            recent_avg = np.mean(
                self.distance_history[-self.progress_smoothing_window :]
            )
            older_avg = (
                np.mean(
                    self.distance_history[
                        -self.progress_smoothing_window
                        * 2 : -self.progress_smoothing_window
                    ]
                )
                if len(self.distance_history) >= self.progress_smoothing_window * 2
                else recent_avg
            )
            ...
            if smoothed_progress > 0:
                progress_reward = min(
                    smoothed_progress / max_distance * 3.0, 1.0
                )  # Capped
            else:
                progress_reward = max(
                    smoothed_progress / max_distance * 1.0, -0.5
                )  # Less penalty
        ...
        if self.oscillation_count > self.oscillation_penalty_threshold:
            excess_oscillations = (
                self.oscillation_count - self.oscillation_penalty_threshold
            )
        ...
        if self.trajectory_efficiency > self.efficiency_bonus_threshold:
            efficiency_bonus = (
                self.trajectory_efficiency - self.efficiency_bonus_threshold
            ) * 1.0
        ...
        if (
            np.linalg.norm(missile_to_target) > 0
            and np.linalg.norm(missile_velocity) > 0
        ):
        ...
        total_reward = (
            distance_reward * 1.0
            + progress_reward * 0.8  # Primary signal
            + time_penalty * 1.0  # Reduced to prevent exploitation
            + oscillation_penalty * 1.0  # NEW: Encourage speed
            + efficiency_bonus * 0.5  # NEW: Discourage oscillation
            + alignment_reward * 0.3  # NEW: Reward efficient paths
            + boundary_penalty * 1.0  # Reduced weight
            + action_smoothness * 1.0  # Unchanged  # NEW: Encourage smooth control
        )
    ...
    def _update_trajectory_efficiency(self):
        ...
        for i in range(1, len(self.distance_history)):
            actual_path_length += abs(
                self.distance_history[i - 1] - self.distance_history[i]
            )
    ...
    def _calculate_boundary_penalty(self) -> float:
        ...
        if self.missile.pos["x"] < margin:
            ...
            penalty -= (
                (self.missile.pos["x"] - (self.screen_width - margin)) / margin * 0.5
            )
        ...
        if self.missile.pos["y"] < margin:
            ...
            penalty -= (
                (self.missile.pos["y"] - (self.screen_height - margin)) / margin * 0.5
            )
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        distance_norm = distance / (
            np.sqrt(self.screen_width**2 + self.screen_height**2)
        )
        ...
        relative_angle_norm = (
            np.arctan2(np.sin(relative_angle), np.cos(relative_angle)) / np.pi
        )
    ...
    def _is_terminated(self) -> bool:
        ...
        if (
            self.missile.pos["x"] < -100
            or self.missile.pos["x"] > self.screen_width + 100
            or self.missile.pos["y"] < -100
            or self.missile.pos["y"] > self.screen_height + 100
        ):
            return True
    ...
    def _get_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            < self.curriculum_stage.success_threshold,
        }
...
class CurriculumManager:
        ...
        if self.stage_episode_count >= self.evaluation_window:
            ...
            if success_rate >= current_stage.min_success_rate:
                ...
                if self.current_stage_idx < len(self.stages) - 1:
                    ...
                    self.stage_success_count = 0
                    logging.info(
                        f"🎓 Advanced to curriculum stage: {self.get_current_stage().name}"
                    )
                    return True
                else:
                ...
                self.stage_success_count = 0
                logging.info(
                    f"📚 Continuing stage '{current_stage.name}' - success rate: {success_rate:.2%}"
                )
    ...
    def get_progress_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "ready_for_next": (
                success_rate >= current_stage.min_success_rate
                and self.stage_episode_count >= self.evaluation_window
            ),
        }
