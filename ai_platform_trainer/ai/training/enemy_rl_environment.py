...
class EnemyRLEnvironment(gym.Env):
    ...
    def _apply_enemy_action(self, action):
        ...
        if speed > self.enemy_max_speed:
            self.enemy_velocity["x"] = (
                self.enemy_velocity["x"] / speed
            ) * self.enemy_max_speed
            self.enemy_velocity["y"] = (
                self.enemy_velocity["y"] / speed
            ) * self.enemy_max_speed
    ...
    def _update_player_heuristic(self):
        ...
        self.player.position["x"] = np.clip(
            self.player.position["x"], 20, self.screen_width - 20
        )
        self.player.position["y"] = np.clip(
            self.player.position["y"], 20, self.screen_height - 20
        )
    ...
    def _handle_missile_spawning(self):
        ...
        if (
            ...
            and self.time_since_last_missile > 30
            and np.random.random()  # Minimum time between missiles
            < self.config.missile_frequency
        ):
            self._spawn_missile()
    ...
    def _update_missiles(self):
        ...
        for missile in self.missiles:
            ...
            if (
                missile.pos["x"] < -50
                or missile.pos["x"] > self.screen_width + 50
                or missile.pos["y"] < -50
                or missile.pos["y"] > self.screen_height + 50
            ):
                missiles_to_remove.append(missile)
    ...
    def _calculate_reward(self, action):
        ...
        for missile in self.missiles:
            ...
            if distance < 100:  # Danger zone
                ...
                if dot_product < 0:  # Moving away from missile
                    evasion_reward = (
                        abs(dot_product) / (8.0 * self.enemy_max_speed) * 1.5
                    )
        ...
        distance_penalty = (
            -abs(distance_to_player - optimal_distance) / optimal_distance * 0.5
        )
        reward += distance_penalty

        # 6. VELOCITY SMOOTHNESS - prefer smooth movements
        velocity_magnitude = math.sqrt(
            self.enemy_velocity["x"] ** 2 + self.enemy_velocity["y"] ** 2
        )
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        obs.extend(
            [
                ...
                min(self.enemy_pos["x"], self.screen_width - self.enemy_pos["x"])
                / self.screen_width,  # Distance to closest x boundary
                min(self.enemy_pos["y"], self.screen_height - self.enemy_pos["y"])
                / self.screen_height,  # Distance to closest y boundary
            ]
        )
    ...
    def _get_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "movement_efficiency": self.survival_time
            / max(1, self.total_distance_moved),
...
class EnemyRLTrainer:
    ...
    def __init__(
        self,
        save_path: str = "models/enemy_rl_sac",
        config: Optional[EnemyRLConfig] = None,
    ):
    ...
    def test_model(self, model_path: str, num_episodes: int = 20):
        ...
        for episode in range(num_episodes):
            ...
            logging.info(
                f"Episode {episode + 1}: Survived {info['survival_time']} steps, "
