...
def run_command(command, shell=False, env=None, capture_output=True):
    ...
    try:
        if capture_output:
            ...
            result = subprocess.run(command, shell=shell, check=False, env=env)
...
def check_python():
    ...
    if major >= 3 and minor >= 7:
        ...
        print_status(
            "Python Version", "warning", f"Python {version} (3.7+ recommended)"
        )
...
def check_cpp_compiler():
    ...
    if platform.system() == "Windows":
        ...
        if os.path.exists(vswhere_path):
            ...
            if code == 0 and out.strip():
                ...
                if os.path.exists(vcvars_path):
                    print_status(
                        "VC Environment Script", "ok", f"Found at {vcvars_path}"
                    )
        ...
        print_status(
            "Visual C++ Compiler", "missing", "Required for building C++ extensions"
        )
        return False
    else:
        ...
        for compiler in ["g++", "clang++"]:
            ...
            if compiler_path:
                ...
                if code == 0:
                    print_status(
                        f"{compiler} Compiler", "ok", f"Found at {compiler_path}"
                    )
                    return True

        print_status("C++ Compiler", "missing", "Required for building C++ extensions")
        return False
    ...
    if cmake_path:
        ...
        if code == 0:
            ...
            if major < 3 or (major == 3 and minor < 18):
                print_status("CMake Version", "warning", "CMake 3.18+ is recommended")
            return True
...
def check_cuda():
    ...
    if nvcc_path:
        ...
        if not cuda_found:
            print_status("CUDA Toolkit", "missing", "Required for GPU acceleration")
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            print_status(
                "PyTorch CUDA Support", "warning", "CUDA not available through PyTorch"
            )
    except ImportError:
        print_status("PyTorch", "missing", "Required for AI training and inference")
...
def check_python_dependencies():
    ...
    for package, min_version in dependencies.items():
        try:
            ...
            print_status(
                f"Python {package}", "missing", f"Minimum version: {min_version}"
            )
            all_found = False
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if returncode == 0:
                ...
                print(
                    "You may need to download and install Visual Studio Build Tools manually."
                )
                return False
    except Exception as e:
    ...
    if platform.system() == "Windows":
        try:
            ...
            returncode, out, err = run_command(
                ["winget", "install", "Kitware.CMake"], capture_output=True
            )
...
def install_cuda():
    ...
    print(
        Colors.colorize(
            "CUDA Toolkit installation requires manual steps.", Colors.YELLOW
        )
    )
    ...
    print(
        "4. After installation, add CUDA bin directory to your PATH environment variable"
    )
...
def main():
    ...
    parser.add_argument(
        "--install", action="store_true", help="Attempt to install missing dependencies"
    )
    parser.add_argument(
        "--cuda-only",
        action="store_true",
    ...
    if all_ok:
        ...
        if args.install:
            ...
            if all_ok:
                ...
                print(
                    "Please refer to SETUP_GUIDE.md for manual installation instructions."
                )
        else:
            print(
