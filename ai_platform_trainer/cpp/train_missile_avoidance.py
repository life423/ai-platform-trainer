...
from gpu_environment import HAS_GPU_ENV, create_vectorized_env, make_env, native_env
...
def train_model(
    ...
    try:
        logger.info(
            f"Starting training with {num_envs} environments for {total_timesteps} timesteps"
        )
...
def evaluate_model(model_path, normalize_path=None, num_episodes=10):
    """
    ...
    for i in range(num_episodes):
        ...
        logger.info(
            f"Episode {i+1}: reward={episode_reward:.2f}, length={steps}, avoidances={avoidances}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train missile avoidance using GPU-accelerated environment"
    )
    parser.add_argument(
        "--num-envs", type=int, default=4, help="Number of parallel environments"
    )
    parser.add_argument(
        "--timesteps", type=int, default=1000000, help="Total timesteps to train for"
    )
    parser.add_argument(
        "--save-dir",
        type=str,
        default="models/gpu_rl",
    ...
    parser.add_argument(
        "--save-freq", type=int, default=10000, help="Frequency to save checkpoints"
    )
    parser.add_argument(
        "--eval", action="store_true", help="Evaluate model instead of training"
    )
    parser.add_argument(
        "--model-path", type=str, help="Path to the model for evaluation"
    )
    parser.add_argument(
        "--eval-episodes",
        type=int,
