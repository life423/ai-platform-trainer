...
from gpu_environment import HAS_GPU_ENV, make_env
...
def monitor_resources():
    ...
    while not stop_monitoring:
        ...
        if int(current_time) % 5 == 0 and current_time > 0:
            logger.info(
                f"GPU: {gpu_util:.1f}%, CPU: {cpu_util:.1f}%, Memory: {mem_usage:.1f}MB"
            )
...
def benchmark_performance(num_envs=4, train_steps=2000, benchmark_cpu=True):
    """
    ...
    try:
        ...
        if gpu_utilization:
            ...
            if max_gpu > 10.0:  # Threshold to consider GPU being used
                ...
                print(
                    "WARNING: Low GPU utilization. CUDA kernels might not be using the GPU effectively."
                )
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Verify GPU utilization during training"
    )
    parser.add_argument(
        "--cuda-check", action="store_true", help="Check CUDA capabilities"
    )
    parser.add_argument(
        "--extension-check", action="store_true", help="Check PyBind11 extension"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--train-steps", type=int, default=2000, help="Number of training steps"
    )
    ...
    if not (
        args.cuda_check or args.extension_check or args.benchmark or args.cuda_test
    ):
        args.all = True
    ...
    if args.all or args.benchmark:
        benchmark_performance(
            train_steps=args.train_steps, benchmark_cpu=not args.no_cpu
        )
