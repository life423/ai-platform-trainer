...
class PlayRLEnemyMode:
    ...
    def __init__(self, game, rl_model_path: str = None):
        ...
        logging.info(
            "🤖 RL Enemy play mode initialized - prepare for intelligent opposition!"
        )
    ...
    def update(self, current_time: int) -> None:
        ...
        if self.game.player:
            ...
            enemy_pos = (
                self.rl_enemy.pos if self.rl_enemy and self.rl_enemy.visible else None
            )
        ...
        if (
            self.rl_enemy
            and hasattr(self.rl_enemy, "fading_in")
            and self.rl_enemy.fading_in
        ):
    ...
    def _respawn_enemy(self):
        ...
        logging.info(
            f"RL enemy respawned at ({x}, {y}) - ready to apply learned strategies"
        )
    ...
    def draw_ui(self, screen: pygame.Surface):
        ...
        # Performance stats
        survival_text = self.font_small.render(
            f"Survival Time: {stats['survival_time']}", True, self.text_primary
        )
        screen.blit(survival_text, (ui_x + 10, y_offset))
        y_offset += 18

        dodged_text = self.font_small.render(
            f"Missiles Dodged: {stats['missiles_dodged']}", True, self.text_primary
        )
        ...
        energy_text = self.font_small.render(
            f"Energy: {stats['energy']:.0f}%", True, self.text_primary
        )
        ...
        energy_color = (
            self.text_secondary if stats["energy"] > 30 else self.warning_color
        )
        pygame.draw.rect(
            screen, energy_color, (bar_x, bar_y, energy_fill_width, bar_height)
        )

        # Draw RL status indicator
        if stats["rl_enabled"]:
            indicator_text = self.font_small.render(
                "TRUE RL ACTIVE", True, self.rl_indicator_color
            )
            screen.blit(indicator_text, (ui_x + 10, ui_y + ui_height - 25))
        else:
            indicator_text = self.font_small.render(
                "FALLBACK MODE", True, self.warning_color
            )
            screen.blit(indicator_text, (ui_x + 10, ui_y + ui_height - 25))
    ...
    def get_mode_info(self) -> str:
        ...
        if self.rl_enemy:
            ...
            if stats["rl_enabled"]:
                return (
                    f"RL Enemy Mode - True AI (Survived: {stats['survival_time']}, "
                    f"Dodged: {stats['missiles_dodged']})"
                )
            else:
