...
class PlayLearningMode:
    ...
    def __init__(self, game):
        ...
        self.ui_bg_color = (
            ...
            112,
            180,
        )  # Semi-transparent midnight blue for contrast
    ...
    def update(self, current_time: int) -> None:
        ...
        if self.game.player:
            ...
            enemy_pos = (
                ...
                if self.learning_enemy and self.learning_enemy.visible
                else None
            )
        ...
        if (
            self.learning_enemy
            and hasattr(self.learning_enemy, "fading_in")
            and self.learning_enemy.fading_in
        ):
    ...
    def _entities_collide(self, entity1, entity2) -> bool:
        ...
        rect1 = pygame.Rect(
            entity1.position["x"], entity1.position["y"], entity1.size, entity1.size
        )
        rect2 = pygame.Rect(
            entity2.pos["x"], entity2.pos["y"], entity2.size, entity2.size
        )
    ...
    def render_learning_ui(self, screen: pygame.Surface):
        ...
        pygame.draw.rect(
            screen, self.progress_bar_bg, (bar_x, bar_y, bar_width, bar_height)
        )
        pygame.draw.rect(
            screen, self.text_secondary, (bar_x, bar_y, bar_width, bar_height), 1
        )
        ...
        difficulty_text = self.font_small.render(
            f"Difficulty: {difficulty:.1%}", True, self.text_primary
        )
        ...
        left_stats = [f"Frames: {stats['frames']:,}", f"Speed: {stats['speed']:.1f}"]
        right_stats = [f"Hits: {stats['hits']}", f"Deaths: {stats['deaths']}"]
        ...
        status_bg = pygame.Surface(
            (status_rect.width + 20, status_rect.height + 8), pygame.SRCALPHA
        )
    ...
    def draw_mode_info(self, screen: pygame.Surface):
        ...
        title_text = self.font_large.render(
            "LEARNING AI MODE", True, self.highlight_color
        )
        title_rect = title_text.get_rect()
        title_bg = pygame.Surface(
            (title_rect.width + 20, title_rect.height + 8), pygame.SRCALPHA
        )
        ...
        for i, instruction in enumerate(instructions):
            instruction_text = self.font_small.render(
                instruction, True, self.text_primary
            )
            instruction_rect = instruction_text.get_rect()

            # Background for each instruction
            inst_bg = pygame.Surface(
                (instruction_rect.width + 16, instruction_rect.height + 4),
                pygame.SRCALPHA,
            )
