...
class EnemyRL:
    ...
    def __init__(
        ...
        if not self.rl_available and fallback_to_scripted:
            from ai_platform_trainer.entities.enemy_learning import (
                AdaptiveStagedEnemyAI,
            )
        ...
        self.visible = True
        self.energy = 100.0
        # Track recent missile positions
        self.last_missile_positions: List[Tuple[float, float]] = []
        ...
        logging.info(
            "🤖 RL Enemy initialized - learning evasion and survival strategies"
        )
    ...
    def update_movement(
        ...
        if self.rl_available and self.rl_model:
            ...
            self.fallback_enemy.update_movement(
                player_x, player_y, player_step, current_time, missiles
            )
    ...
    def _create_rl_observation(
        ...
        obs.extend(
            [
                ...
                min(self.pos["x"], self.screen_width - self.pos["x"])
                / self.screen_width,
                min(self.pos["y"], self.screen_height - self.pos["y"])
                / self.screen_height,
            ]
        )
    ...
    def _basic_evasion(self, player_x: float, player_y: float, missiles: List):
        ...
        if (
            ...
            or self.pos["x"] > self.screen_width - edge_margin
            or self.pos["y"] < edge_margin
            or self.pos["y"] > self.screen_height - edge_margin
        ):
    ...
    def _track_missile_evasion(self, missiles: List):
        ...
        if len(self.last_missile_positions) > len(current_missile_positions):
            missiles_lost = len(self.last_missile_positions) - len(
                current_missile_positions
            )
    ...
    def draw(self, screen: pygame.Surface):
        ...
        pygame.draw.ellipse(enemy_surface, color, (0, 0, self.size, self.size))
        ...
        if self.rl_available:
            ...
            pygame.draw.circle(enemy_surface, indicator_color, (self.size - 5, 5), 3)
    ...
    def get_ai_info(self) -> str:
        ...
        if self.rl_available:
            return (
                f"True RL Enemy (SAC) - Survival: {self.survival_time}, "
                f"Dodged: {self.missiles_dodged}"
            )
        elif self.fallback_enemy:
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        movement_efficiency = self.survival_time / max(
            1, self.survival_time * 0.1
        )  # Rough estimate
