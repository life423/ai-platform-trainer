...
def calculate_enhanced_reward(
    distance, steps, oscillations, trajectory_efficiency, progress
):
    ...
    smoothed_progress = (
        min(progress * 0.8, 1.0) if progress > 0 else max(progress * 0.5, -0.5)
    )
...
for scenario in scenarios:
    old_reward = calculate_old_reward(
        scenario["distance"], scenario["steps"], scenario["progress"]
    )
    ...
    print(
        f"{scenario['name']:<25} {old_reward:<12.3f} {enhanced_reward:<12.3f} {improvement:<12.3f}"
    )
...
old_rewards = [
    calculate_old_reward(s["distance"], s["steps"], s["progress"]) for s in scenarios
]
enhanced_rewards = [
...
ax1.bar(
    ...
    enhanced_rewards,
    width,
    label="Enhanced Reward Function",
    alpha=0.7,
    color="blue",
)
...
print(
    "   This will train with all improvements and generate detailed analysis reports!"
)
